...
var diff = function (callback) {
  var currents = fs.readdirSync(constant.currentPath);
  var ep = new EventProxy();
  ep.after('diff', currents.length, function (list) {
    callback(list);
  });
  currents.forEach(function (filename) {
    var newer = path.join(constant.currentPath, filename);
    var older = path.join(constant.referencePath, filename);
    var output = path.join(constant.diffPath, filename);
